...
const createCssVar = (before = '--') => {
  const transformCssVars: (items: unknown, prefix?: string) => string[] = (items, prefix = '-') =>
...
export const createCssVars: (themeColors: unknown, before?: string) => string = (themeColors, before = '--') =>
